...
const userId = req.user?.id
    ...
    if (req.file) {
      const result = await uploadToCloudinary(
        req.file.buffer,
        "turfarena/superadmins",
        `super-admins_${userId}_${Date.now()}`
      );
...
exports.updateUser = async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      "SELECT id, photo FROM super_admins WHERE id = ?", [userId]
    );
    if (!rows.length)
    ...
    await db.query(
      `UPDATE super_admins SET ${fields.join(", ")} WHERE id = ?`,
      [...values, userId]
    );
...
exports.deleteAdminPhoto = async (req, res) => {
  try {
    ...
    // Clear from DB first
    await db.query("UPDATE super_admins SET photo = NULL WHERE id = ?", [userId]);
    ...
    console.log(`[Super admin-photo] removed admin=${userId}`);
...
exports.changePassword = async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      "SELECT password FROM super_admins WHERE id = ?", [userId]
    );
    if (!rows.length)
    ...
    await db.execute(
      "UPDATE super_admins SET password = ? WHERE id = ?", [hashed, userId]
    );
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      "SELECT id FROM super_admins WHERE reset_token = ? AND reset_token_expiry > NOW()",
      [token]
    );
    if (!rows.length)
    ...
    const hashed = await bcrypt.hash(newPassword, 10);
    await db.execute(
      `UPDATE super_admins SET password = ?, reset_token = NULL, reset_token_expiry = NULL WHERE id = ?`,
      [hashed, rows[0].id]
    );
