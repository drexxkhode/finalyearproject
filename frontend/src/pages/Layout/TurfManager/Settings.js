...
function ConfirmModal({ show, message, onConfirm, onCancel, loading }) {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ fontSize: 44, lineHeight: 1 }} className="mb-3">
          🗑️
        </div>
...
const Settings = () => {
  ...
  const handleTurfDetailSubmit = async (e) => {
    ...
    setUpdating(true);
    const flushedAmenities = amenitiesRaw
      .split(",")
      .map((a) => a.trim())
      .filter(Boolean);
  ...
  const handleImageReplace = async (e) => {
    ...
    try {
      ...
      setImages((prev) =>
        prev.map((img) => {
          if (img.id === imageId)
            return { ...newImage, is_cover: wascover ? 1 : newImage.is_cover };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setImages((prev) => {
        ...
        const deletedWasCover = prev.find(
          (img) => img.id === imageId,
        )?.is_cover;
              ...
              <div className="custom-tabs-container">
                ...
                <div className="tab-content">
                  ...
                  >
                    ...
                    {/* ── Save Changes — now below both cards, still inside twoA ── */}
                      ...
                      <div className="card-body">
                        ...
                        {!loadingImages && images.length === 0 && (
                          <div className="text-center py-5 text-muted">
                            ...
                            <p className="small">
                              Upload images using the button above. The first
                              image automatically becomes the cover.
                            </p>
                          </div>
                        )}
                        {!loadingImages && images.length > 0 && (
