...
const EditSuperAdmin = () => {
  ...
  const fetchDetail = async () => {
    try {
      ...
      const res = await axios.get(`${API}/api/super/details/${id}`, {
  ...
  const refreshLoggedInUser = async () => {
    ...
    if (!token) return;
    try {
      const res = await axios.get(`${API}/api/super/me`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const freshUser = {
        ...res.data,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(`${API}/api/super/update/${id}`, payload, {
        headers: { Authorization: `Bearer ${token}` },
            ...
            <div className="card-body">
              <div className="custom-tabs-container">
                <ul className="nav nav-tabs">
                  {[{ key: "oneA", label: "Edit General" }].map(
                    ({ key, label }) => (
                          ...
                          >
                            ...
                            <div className="d-flex align-items-center gap-4">
                              <img
                                src={
                                  previewImage ||
                                  `${process.env.PUBLIC_URL}/assets/images/admin/avatar.webp`
                                }
                                className="profile-img"
                                alt="Profile"
                                onError={(e) => {
                                  e.target.src = `${process.env.PUBLIC_URL}/assets/images/admin/avatar.webp`;
                                }}
                              />
                              {errors.photo && (
