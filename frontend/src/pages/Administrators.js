...
const Administrators = () => {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const { data } = await axios.get(`${API}/api/auth/admins`, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`${API}/api/auth/delete/${id}`, {  // ← fixed: was /api/admins/${id}
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchAdmins();
    } catch (err) {
    ...
    <div className="col-xxl-12">
      <div className="card mb-3">
        <div className="card-header d-flex justify-content-between align-items-center">
          ...
          <Link className="btn btn-primary btn-sm ms-auto" to="/register" title="Register New User">
              ...
              <tbody>
                {admins.map((admin, index) => (
                  <tr key={admin.id}>
                    ...
                    <td>{`${admin.firstName} ${admin.middleName || ""} ${admin.lastName}`.trim()}</td>
                    <td>{admin.email}</td>
                    <td>{admin.contact}</td>
                    <td>{admin.role}</td>
                    <td>
                      ...
                      <Link
                        ...
                        title="View Profile"
                      >
                        View <i className="bi bi-arrow-right-circle" />
                      </Link>
                    </td>
                  </tr>
                ))}
