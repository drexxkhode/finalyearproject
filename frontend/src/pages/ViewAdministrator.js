...
const Update = () => {
  ...
  const fetchDetail = async (adminId) => {
    try {
      ...
      const { data } = await axios.get(
        `http://localhost:5000/api/auth/details/${adminId}`,
      ...
      setFormData({
        ...
        role: data.role || "",
        photo: null, // Keep null, will handle upload separately
      });
  ...
  return () => {
    ...
    if (isNewPhoto && previewImage) {
      URL.revokeObjectURL(previewImage);
    }
  };
}, [previewImage, isNewPhoto]);
  // ------------------- Input Handling -------------------
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/auth/update/${id}`, payload, {
        headers: {
