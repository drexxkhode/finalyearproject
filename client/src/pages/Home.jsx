...
export default function Home({
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API}/turf/turf-data`, { headers })
      .then((res) => {
        setTurfs(
          res.data.data.map((t) => ({
            ...t,
...
const rec = [...turfs].sort((a, b) => recommendationScore(b) - recommendationScore(a));
  ...
  if (!turfs.length)
    return <p className="text-center py-5 text-muted">No turfs available</p>;
  ...
  return (
    ...
    <div className={animated ? "tf-animate-in" : ""}>
          ...
          <div className="tf-filter-bar">
            <div className="tf-filter-search">
              <input
                ...
                placeholder="🔍 Search turfs in Dome…"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              />
            </div>
            <select
              className="form-select tf-filter-select"
              value={fCap}
            ...
            <div className="d-flex justify-content-center align-items-center gap-3 mt-5">
              ...
              <div className="d-flex align-items-center gap-2">
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((p, idx) =>
                    p === "..." ? (
                      <span key={`e${idx}`} className="text-muted px-1">
                        …
                      </span>
                    ) : (
                      <button
      ...
      {/* ── MAP TAB ── */}
      {activeTab === "map" && <MapView turfs={turfs} onOpenTurf={onOpenTurf} />}
            ...
            <div className="card border-0 shadow-sm rounded-4 overflow-hidden h-100">
              ...
              >
                ...
                <div
                  style={{
                    ...
                    inset: 0,
                    background:
                      "linear-gradient(to bottom, transparent 40%, rgba(0,0,0,.65))",
                  }}
                />
              ...
              <div className="p-3">
                ...
                <div className="d-flex justify-content-between align-items-center">
                  <div>
                    <span className="tf-info-price-big">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                ...
                {recTotal > 1 && (
                  <small className="text-muted">
                    {(recPage - 1) * REC_PAGE_SIZE + 1}–
                    {Math.min(recPage * REC_PAGE_SIZE, rec.length)} of{" "}
                    {rec.length}
                  </small>
                )}
              </div>
